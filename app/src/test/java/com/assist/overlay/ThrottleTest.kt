...
class ThrottleTest {
    ...
    @Test
    fun `a burst within one window collapses to leading plus trailing`() = runTest {
    ...
    @Test
    fun `values spaced beyond the window are each emitted`() = runTest {
        val source = flow {
            emit(1)
            delay(200L)
            emit(2)
    ...
    @Test
    fun `the final value is never dropped`() = runTest {
    ...
    @Test
    fun `non-positive period is a pass-through`() = runTest {
