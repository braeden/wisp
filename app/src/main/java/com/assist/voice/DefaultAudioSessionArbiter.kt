 ...
 */
@Singleton
class DefaultAudioSessionArbiter @Inject constructor() : AudioSessionArbiter {

    private val lock = Mutex()
    private var holder: Token? = null

    private class Token(val owner: MicOwner) {
    ...
    override suspend fun <T> withMic(owner: MicOwner, block: suspend () -> T): T {
        ...
        try {
            return coroutineScope {
                ...
                val watcher = launch {
                    ...
                    work.cancel(CancellationException("mic preempted by higher-priority owner"))
                }
                try {
                    work.await()
                } finally {
    ...
    private suspend fun acquire(owner: MicOwner): Token {
        while (true) {
            val waitOn: Token? = lock.withLock {
                val current = holder
                when {
                    current == null -> {
