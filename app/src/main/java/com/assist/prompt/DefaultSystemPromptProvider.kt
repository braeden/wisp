 ...
 */
@Singleton
class DefaultSystemPromptProvider @Inject constructor() : SystemPromptProvider {
    ...
    companion object {
        ...
        val STABLE_CORE: String = """
            ...
            get faster and cheaper at repeated tasks:
            - **Check memory first.** Before working out a task from scratch, look
              under `/memories/tasks/` for a recipe matching the intent and follow it
              if present (adapting to the current UI, which may have shifted).
            - **Record a recipe after finishing a non-trivial, novel task.** Write a
